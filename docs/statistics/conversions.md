...
- [Загрузка данных о конверсиях](ru/statistics/conversions#data-load)
- [Использование идентификаторов JavaScript-целей в качестве статуса заказа](ru/statistics/conversions#javascript-goal)
- [Окно атрибуции конверсии к визитам и окно на изменении информации о конверсии](ru/statistics/conversions#first-load)
- [Как проверить, что все работает?](ru/statistics/conversions#сheck-work)
- [Ошибки при загрузке](ru/statistics/conversions#mistake)
- [Ошибки в файле](ru/statistics/conversions#errors_file)
- [Статистика по конверсиям](ru/statistics/conversions#stat)
...
1. [Сформируйте файл с данными по установленному формату](ru/statistics/conversions#step1).
2. [Загрузите файл в источник](ru/statistics/conversions#step2).
3. [Добавьте ссылку на источник](ru/statistics/conversions#step3).
...
1. Если передаете как онлайн, так и офлайн-заказы, убедитесь, что в созданной выгрузке есть оба вида заказов. Для этого возьмите несколько конкретных заказов (онлайн, офлайн, звонки) и проверьте, что они с правильными контактами попали в выгрузку. В выгрузке есть проблемы, если у всех заказов:
   ...
   Подробнее про [качество атрибуции](ru/strategies/offline-conversions).
...
| Invalid '<id\_type>' identifier '<id\_value>' | В колонке с идентификаторами найдено некорректное значение. Укажите идентификатор пользователя в корректном формате. Подробнее про [формат телефона и почты](ru/statistics/conversions#data-load). Clientid должен быть указан как полное положительное число |
...
- **Конверсии из Директа** — количество и доля конверсий, которые случились после перехода на сайт по кампаниям из Яндекс Директа.
...
[Офлайн-конверсии](ru/strategies/offline-conversions)
