...
> Например, вы можете посмотреть запросы, по которым были показаны ваши объявления. В блоке **Срезы** выберите **№ баннера** и **Поисковый запрос**, в блоке **Столбцы** — **Клики** и **Отказы**. Чтобы исключить показы по запросам, которые кажутся вам неподходящими, используйте [минус-фразы](ru/keywords/negative-keywords). Их можно добавить на странице редактирования кампании или группы баннеров.
